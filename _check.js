...
const flowTemplates = {
  ...
  '周期保养': {
    master: ['待受理','维修中','待验收','已完成'],
    sub: ['待服务站接单','待配件','已预约','已到场','保养开始','保养完成']
  },
  '软件复诊': {
    master: ['待受理','复诊中','待验收','已完成'],
...
function statusTag(s){
  var m={'待受理':'pending','维修中':'doing','待验收':'verify','已完成':'done','受理驳回':'rejected'};
  return '<span class="tag tag-'+(m[s]||'status')+'">'+s+'</span>';
}
...
function getSlaText(order){
  if(order.status==='已完成'||order.status==='受理驳回') return '-';
...
function updateStatCards(){
  var keys = ['待受理','维修中','待验收','已完成','受理驳回'];
  keys.forEach(function(k){
    var el = document.getElementById('cnt-'+k);
    if(el) el.innerText = orders.filter(function(o){ return o.status===k; }).length;
  });
}
renderTable(orders);
initCascader();
    ...
    '<div class="detail-page-body-wrap">' +
      ...
      '<div class="detail-card">' +
        ...
        '<div class="info-grid">' +
          ...
          '<div class="info-item"><span class="info-label">工单状态</span><span class="info-val">' + (o.status||'—') + '</span></div>' +
...
function filterByCard(el, status){
  ...
  activeCard = status;
  el.classList.add('active');
  var list = orders.filter(function(o){ return o.status === status; });
  renderTable(list);
}
...
function openDrawer(id){
  ...
  var actions = '';
  if(o.status==='待受理') actions = '<button class="btn btn-danger" onclick="opAct(\''+o.id+'\',\'驳回\')">受理驳回</button><button class="btn btn-primary" onclick="opAct(\''+o.id+'\',\'受理\')">受理并创建子单</button><button class="btn btn-default" onclick="openEditModal(\''+o.id+'\')">编辑工单</button>';
  else if(o.status==='待验收') actions = '<button class="btn btn-danger" onclick="opAct(\''+o.id+'\',\'验收驳回\')">验收驳回</button><button class="btn btn-success" onclick="opAct(\''+o.id+'\',\'验收通过\')">验收通过</button><button class="btn btn-default" onclick="openEditModal(\''+o.id+'\')">编辑工单</button>';
  ...
  else if(o.status==='已完成' || o.status==='受理驳回') actions = '';
...
function submitNew(){
  ...
  orders.unshift({id:id, vehicle:v, site:s, type:t, pri:p, status:'待受理', sub:'-', station:'待推荐', sla:'剩余 60 分钟', next:'运营受理', desc:document.getElementById('newDesc').value,
...
function renderMaintPlan(list){
  ...
  document.getElementById('maintPlanBody').innerHTML = currentPlanList.map(function(r,i){
    ...
    return '<tr style="'+rowStyle+'">'
      ...
      +'<td>'+r.lastTime+'</td><td>'+r.lastMileage+'</td><td>'+r.planType+'</td>'
...
function applyPlanFilter(){
  ...
  var t=document.getElementById('mpType').value;
  var u=document.getElementById('mpUrgency').value;
  renderMaintPlan(maintPlan.filter(function(p){ return (!kw||p.vehicle.includes(kw)||p.site.includes(kw))&&(!t||p.planType===t)&&(!u||(u==='已提单'?!!p.hasOrder:p.urgency===u)); }));
}
function resetPlanFilter(){ ['mpKw','mpType','mpUrgency'].forEach(function(i){document.getElementById(i).value='';}); activeMaintCard=null; document.querySelectorAll('#maintStatCards .stat-card').forEach(function(c){c.classList.remove('active');}); renderMaintPlan(); }
